...
class StandCarFormController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->addColumn('car_year', function ($row) {
                return $row->car && $row->car->catalogYear ? $row->car->catalogYear->name : '';
            });
    ...
    public function create()
    {
        ...
        $cars = StandCar::with(['brand', 'car_model', 'catalogYear'])
            ->get()
            ->mapWithKeys(function ($car) {
                $label = trim(collect([
                    optional($car->brand)->name,
                    optional($car->car_model)->name,
                    optional($car->catalogYear)->name,
                ])->filter()->join(' '));
                return [$car->id => $label ?: $car->id];
            })
