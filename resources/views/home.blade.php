            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    @if(($driver_card_codes ?? collect())->isEmpty() && !$hasOtherFuelTransactions)
                        <div class="alert alert-info">Sem cartões associados ao motorista.</div>
                    @elseif(!$hasCombustionTransactions && !$hasOtherFuelTransactions)
                        <div class="alert alert-info">
                            ...
                            <br>
                            <small>Cartões considerados: {{ $driver_card_codes->join(', ') }}</small>
                            @endif
                        </div>
                    @else
                        @if(($driver_card_codes ?? collect())->isNotEmpty())
                        <p><small>Cartões considerados: {{ $driver_card_codes->join(', ') }}</small></p>
                        @endif
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                @foreach($combustion_transactions as $tx)
                                    @php
                                        $unit = $card_units[$tx->card] ?? 'L';
                                    @endphp
                                    <tr>
                            ...
                            <table class="table table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="text-align:left;">Tipo de cartão</th>
                                        <th style="text-align:right;">Custo (€)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($other_fuel_transactions as $tx)
                                        <tr>
                                            ...
                                            <td>{{ number_format((float)$tx->value, 2, ',', ' ') }}€</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                            </table>
        ...
        <div class="col-md-7">
            <div class="panel panel-default">
                <div class="panel-body">
                    ...
                    <h3>Saldo atual: <span style="font-weight: 800;">{{ $driver_balance->new_balance ?? 0 }}</span>€</h3>
                    </div>
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <table class="table table-striped">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ number_format($driver_balance_last_week->new_balance ?? 0, 2) }}€</td>
                            </tr>
                            @endif
                            <tr>
                                <th>Saldo atual</th>
                    ...
                    @if ($driver_balance && $driver_balance->new_balance > 0)
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group {{ $errors->has('value') ? 'has-error' : '' }}">
                                    ...
                                    <input class="form-control" type="hidden" name="value" id="value" value="{{ number_format((float)$driver_balance->new_balance, 2, '.', '') }}">
                                    <input class="form-control" type="text" disabled value="{{ $driver_balance->new_balance }}" placeholder="Verifique o seu recibo para confirmar o valor." required>
                                    @if($errors->has('value'))
