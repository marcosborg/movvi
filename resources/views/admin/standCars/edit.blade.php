        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.standCar.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.stand-cars.update", [$standCar->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('brand') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="brand_id" id="brand_id" required>
                                @foreach($brands as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('brand_id') ? old('brand_id') : $standCar->brand->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('brand'))
                        ...
                        <div class="form-group {{ $errors->has('car_model') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="car_model_id" id="car_model_id" required>
                                @foreach($car_models as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('car_model_id') ? old('car_model_id') : $standCar->car_model->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('fuel') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="fuel_id" id="fuel_id" required>
                                @foreach($fuels as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('fuel_id') ? old('fuel_id') : $standCar->fuel->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('fuel'))
                        ...
                        <div class="form-group {{ $errors->has('transmision') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\StandCar::TRANSMISION_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="transmision_{{ $key }}" name="transmision" value="{{ $key }}" {{ old('transmision', $standCar->transmision) === (string) $key ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('cylinder_capacity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="cylinder_capacity" id="cylinder_capacity" value="{{ old('cylinder_capacity', $standCar->cylinder_capacity) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('battery_capacity') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="battery_capacity" id="battery_capacity" value="{{ old('battery_capacity', $standCar->battery_capacity) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('year_id') ? 'has-error' : '' }}">
                            <label class="required" for="year_id">{{ trans('cruds.standCar.fields.year_select') }}</label>
                            <select class="form-control select2" name="year_id" id="year_id" required>
                                @foreach($years as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('year_id') ? old('year_id') : $standCar->catalogYear->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('year_id'))
                                <span class="help-block" role="alert">{{ $errors->first('year_id') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.standCar.fields.year_select_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('kilometers') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="kilometers" id="kilometers" value="{{ old('kilometers', $standCar->kilometers) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('power') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="power" id="power" value="{{ old('power', $standCar->power) }}" step="1" required>
                            @if($errors->has('power'))
                        ...
                        <div class="form-group {{ $errors->has('origin') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="origin_id" id="origin_id" required>
                                @foreach($origins as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('origin_id') ? old('origin_id') : $standCar->origin->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('distance') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="distance" id="distance" value="{{ old('distance', $standCar->distance) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $standCar->price) }}" step="0.01">
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="status_id" id="status_id" required>
                                @foreach($statuses as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('status_id') ? old('status_id') : $standCar->status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
